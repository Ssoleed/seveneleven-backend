...
app.get('/confirm.html', (req, res) => {
  // Redirect to Netlify confirm page for booking completion
...
app.post('/booking', (req, res) => {
  ...
  // Check overlap
  const existing = db.prepare(`SELECT * FROM bookings WHERE date = ?`).all(date);
  const isOverlap = existing.some(booking => {
  ...
  // ✅ Redirect to Netlify confirm page
  res.redirect('https://my711.netlify.app/confirm.html');
});
